...
class Quarry(Building):
    ...
    def draw(self, screen, xsize, ysize):
        ...
        numstone = sum(
            [c.amount for c in self.loc.cargo if isinstance(c, cargo.Stone)])
        self.drawText("%d - %d" %
                      (self.capacity, numstone), screen, xsize, ysize)
...
class LumberCamp(Building):
    ...
    def draw(self, screen, xsize, ysize):
        Building.draw(self, screen, xsize, ysize)
        numwood = sum(
            [c.amount for c in self.loc.cargo if isinstance(c, cargo.Wood)])
        self.drawText("%d - %d" %
                      (self.capacity, numwood), screen, xsize, ysize)
...
class StoneMason(Building):
    ...
    def turn(self):
        ...
        numblocks = sum(
            [c.amount for c in self.loc.cargo if isinstance(c, cargo.Blocks)])
        if numblocks > 20:
            self.requires = []
        else:
            self.requires = [cargo.Stone]
...
class Carpenter(Building):
    ...
    def turn(self):
        ...
        # Shutdown if we have surplus production
        numtimber = sum(
            [c.amount for c in self.loc.cargo if isinstance(c, cargo.Timber)])
        if numtimber > 20:
            self.requires = []
        else:
            self.requires = [cargo.Wood]
...
class BuildingSite(Building):
    ...
    def draw(self, screen, xsize, ysize):
        ...
        self.drawText("T%d - B%d" %
                      (numtimber, numblocks), screen, xsize, ysize)
